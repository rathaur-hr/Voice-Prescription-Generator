...
def extract_list_items(transcript: str):
    """
    Generic helper for turning a spoken list (e.g. tests advised) into
    separate items. Splits on commas, semicolons, newlines, and the word
    'and', then title-cases each item. Used for the Tests section, where
    we don't need drug-specific parsing — just a clean bullet list.
    """
    if not transcript or not transcript.strip():
        return []

    # Normalize " and " into a comma so it splits the same way
    normalized = re.sub(r"\band\b", ",", transcript, flags=re.IGNORECASE)
    ...
    for item in raw_items:
        cleaned = item.strip(" .")
        if cleaned:
            items.append(cleaned[0].upper() + cleaned[1:])
    return items
