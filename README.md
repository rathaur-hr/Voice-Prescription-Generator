...
Doctor speaks (or uploads audio) → **Whisper** (offline) transcribes it →
...
Requires Python 3.10 or 3.11 (Whisper/torch don't yet support the newest
Python releases well) and `ffmpeg` installed on your system.
...
> First run will download the Whisper "base" model (~140 MB) — this is a
...
4. Click **Deploy**. Streamlit Cloud will automatically:
   ...
   First deploy takes a few minutes (downloading torch/Whisper). Subsequent
   redeploys (after a `git push`) are much faster.

> **Free-tier note:** Streamlit Community Cloud gives ~1 GB RAM. The app is
> configured to use Whisper's **"base"** model, which fits comfortably. If
> you need higher accuracy and have access to a paid host (Render, AWS,
> a VM, etc.), switch to `"small"` or `"medium"` in `app.py`
